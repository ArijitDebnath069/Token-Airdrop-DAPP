...
export const CONTRACT_ADDRESS =
  "CBXY7ICF7XCUX72PPZYXEATHVIFJFSXTMUGKKCXPT5URXK5SNATFXIEY";
...
// ============================================================
// Token Airdrop — Contract Methods
// ============================================================

/**
 * Airdrop an equal `amount` of `token` from `sender` to every recipient.
 * Calls: airdrop(sender: Address, token: Address, recipients: Vec<Address>, amount: i128)
 */
export async function airdrop(
  ...
  const recipientsScVal = xdr.ScVal.scvVec(
    recipients.map((r) => toScValAddress(r))
  );
  return callContract(
    "airdrop",
    [
 ...
 * Calls: airdrop_custom(sender: Address, token: Address, recipients: Vec<Address>, amounts: Vec<i128>)
 */
export async function airdropCustom(
  caller: string,
  tokenAddress: string,
  recipients: string[],
  amounts: bigint[]
...
/**
 * Get how many airdrops have been run for a given token (read-only).
 * Calls: get_airdrop_count(token: Address) -> u32
 */
export async function getAirdropCount(
  tokenAddress: string,
  caller?: string
): Promise<number> {
...
/**
 * Get total amount of a token distributed through this contract (read-only).
 * Calls: get_total_distributed(token: Address) -> i128
 */
